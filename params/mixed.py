            ...
            "areaList": {
                ...
                "mediaEle": [  # 区域属性
                        ...
                        "srcGroup": [  # 图片区域集合,该区域如果包含多个图片，显示时，将以轮播方式进行播放显示
                            {
                                ...
                                "src":
                                "https://www.ledcloud.cn/resources/picture/A_36_41_1021/202011/4762a18f-4c77-49e3-abcc-9f012692328a_1605322599072.jpg",  # 数据类型text,图片地址，格式为http超链接地址路径（注：务必保证所设地址是有效的http地址,否则，会引起节目播放时黑屏）
                                "dateSW":
                    ...
                    },
                    {
                        ...
                        1,  # 数据类型int,显示的层级顺序 
                        "siderType":
                        "6",  #数据类型vachar[2],特技动效，提供随机、左右、百叶窗左右、百叶窗上下、上下、随机线条、无特效等7种选择，对应取值分别为0,1,2,3,4,5,6
                        "pauseTime":
                        ...
                        "srcGroup": [  # 视频区域集合,该区域如果包含多个视频，显示时，将以轮播方式进行播放显示
                            {
                                ...
                                "src":
                                "https://www.ledcloud.cn/resources/video/A_234_235/202011/be87e372-d256-4c97-ae08-837895ec4407_1605146633213_copy.mp4",  # 数据类型text,视频文件地址，格式为http超链接地址路径（注：务必保证所设地址是有效的http地址,否则，会引起节目播放时黑屏）
                                "dateSW":
                                0,  # 数据类型int,（注：前提条件，节目播放次数开关【playTimesFlag】必须为打开状态，设置才生效）播放时间开关，0为关闭，1为打开状态。默认为关闭状态，即默认不按播放次数播放;当设置为打开状态，即表示该媒体必须按照规定时间范围，每天按照指定的播放次数进行播放，因此，endDate，startDate，playTimes，pauseTime这几个字段，必须设置为有效值，才会生效。如果播放时间范围过期后，则停止播放。
                                "playTimes": 0,  # 数据类型int,播放次数，默认为0
                                "endDate":
                            ...
                            },
                            {
                                ...
                                "size": 3396700,  # 数据类型long,图片文件大小，单位为byte
                                "src":
                                "https://www.ledcloud.cn/resources/picture/A_36_41_1021/202011/d82d8547-1e3e-444b-992b-9cc6577f61e6_1605322599162.jpg",  # 数据类型text,图片地址，格式为http超链接地址路径（注：务必保证所设地址是有效的http地址,否则，会引起节目播放时黑屏）
                                "dateSW":
                                0,  # 数据类型int,（注：前提条件，节目播放次数开关【playTimesFlag】必须为打开状态，设置才生效）播放时间开关，0为关闭，1为打开状态。默认为关闭状态，即默认不按播放次数播放;当设置为打开状态，即表示该媒体必须按照规定时间范围，每天按照指定的播放次数进行播放，因此，endDate，startDate，playTimes，pauseTime这几个字段，必须设置为有效值，才会生效。如果播放时间范围过期后，则停止播放。
                                "playTimes": 0,  # 数据类型int,播放次数，默认为0
                                "endDate":
