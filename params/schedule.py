import time
from utils.functions import datetime_to_timestamp, today_timestamp
    ...
    "programPlan": [  # 节目清单计划属性
        {
            ...
            "program": [  # 节目list
                {
                    "createDate":
                    int((time.time() * 1000)),  # 数据类型long,节目创建时间
                    "date": [  # 节目日程（轮播节目没有日程限制）
                        {
                            "dateStart": today_timestamp(),  # 数据类型long, 起始日期
                            "dateEnd":
                            today_timestamp() + 86400000,  # 数据类型long, 截止日期
                        }
                    ],
                    "isDefault":
                    ...
                    "updateDate":
                    int((time.time() * 1000))  # 数据类型long,节目更新时间
                }
            ]
        }
    ]
